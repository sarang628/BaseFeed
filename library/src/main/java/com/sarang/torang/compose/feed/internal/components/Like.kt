...
@Composable
fun Like(modifier: Modifier = Modifier, isLike: Boolean = false, onLike: () -> Unit = {}, animation: Boolean = true) {
...
@Composable
private fun AnimationLikeImage(modifier: Modifier = Modifier, onLike: () -> Unit = {}, size: Dp = 50.dp, padding: Dp = 0.dp, onFinishAnimation: () -> Unit = {} ) {
    ...
    Icon(modifier = modifier.size(size).padding(padding).graphicsLayer(scaleX = scale.value, scaleY = scale.value).nonEffectclickable { onLike.invoke() },
...
fun PreviewLike(){
    ...
    Box{
        Like(modifier = Modifier.align(Alignment.Center), isLike = isLike, onLike = {isLike = !isLike})
    }
}